...
export class Player {
  ...
  private canMove = (map: IMap) => {
    return (
      !hasWallCollision(this.getCoordinates(), map, this.direction) ||
      (Boolean(this.nextTurn) &&
        !hasWallCollision(this.getCoordinates(), map, this.nextTurn))
    );
  };
  move = (
    map: IMap,
    onEatingDot: (data: { col: number; row: number }) => void
  ) => {
    if (this.canMove(map)) {
