import { BRICKS_COUNT, COLLISION_DISTANCE, ENEMY_COLORS, TILE_SIZE, WALL_ZONE } from "../maps/constants"
...
export const hasWallCollision = (coords: Coordinates, map: IMap, direction: Direction): boolean => {
    const { x, y } = coords;
    if (direction === "Down") {
        //is map end reached
...
export const getEnemyCollisionID = (player: Player, enemies: Array<Enemy>): number | null => {
    let result = null;
    for (let i = 0; i < enemies.length; i++) {
        if (distanceBetweenDots(player.getCoordinates(), enemies[i].getCoordinates()) <= COLLISION_DISTANCE) {
